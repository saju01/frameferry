'use strict';
// Shared across discovery, downloads, handles and runs. Never reset a denial or
// rolling-hour usage to make a job fit. Only downward overrides are supported.
const fs = require('node:fs');
const path = require('node:path');
const crypto = require('node:crypto');
const { ArchiveError } = require('./index.js');
const SESSION_MAX = 120, HOUR_MAX = 140, HOUR_MS = 3600000;
function cap(value, maximum) {
  const n = value === undefined ? maximum : value;
  if (!Number.isInteger(n) || n < 1 || n > maximum) throw new ArchiveError('BAD_BUDGET', 'request limit must be a positive integer no greater than '+maximum);
  return n;
}
...
function openBudget(file, runId, limit) {
  ...
  const maximum=cap(limit,SESSION_MAX); file=path.resolve(file);
  ...
  try {
    ...
    d.session_ceiling=Math.min(maximum,d.session_ceiling||maximum);d.hourly_ceiling=HOUR_MAX;
    ...
    const assert=()=>{if(stopped)throw stopped;if(d.denial)throw new ArchiveError('PROVIDER_DENIED','recorded provider denial '+d.denial.id+' remains in force');};
    ...
    const reserve=phase=>{
      ...
      if(d.requests>=d.session_ceiling || d.recent_request_ms.length>=HOUR_MAX){d.blocked++;save();throw fail('REQUEST_LIMIT','request allowance exhausted; incomplete scope retained');}
    ...
    const fetch=async(url,init)=>{reserve('download');const response=await globalThis.fetch(url,init);try{inspect(response.status,response.headers,url);if(String(response.headers.get('content-type')).includes('text/html')){d.denial={id:crypto.randomBytes(6).toString('hex'),kind:'DENIED_CONTENT_WALL',at:new Date().toISOString(),session_id:runId};save();throw fail('PROVIDER_DENIED','media endpoint returned HTML');}}catch(e){await response.body?.cancel().catch(()=>{});throw e;}return response;};
    save();return {data:d,reserve,inspect,assert,fetch,close,fail};
...
module.exports={openBudget,SESSION_MAX,HOUR_MAX};
