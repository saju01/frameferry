...
async function readManifest(out){ return JSON.parse(await fsp.readFile(path.join(out,'.instacognito','example','manifest.json'),'utf8')); }
...
test('429 Retry-After beyond budget persists DEFERRED and preserves prior completed evidence', async () => {
  ...
  const status=JSON.parse(await fsp.readFile(path.join(out,'.instacognito','example','status.json'),'utf8'));
...
test('CLI doctor parses --attach-cdp without swallowing it as handle, and bad flags fail', () => {
  const bin=path.join(__dirname,'..','bin','instacognito-archive.js');
