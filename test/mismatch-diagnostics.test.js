...
test('sanitizeFailedItem propagates mismatchDiagnostics when gated by errorCode', () => {
  const item = {
    ...
    _mismatchDiagnostics: { expectedMediaType: 'video', actualMediaType: 'image', mimeClass: 'image', contentTypeToken: 'image/jpeg', httpStatus: 200, declaredLength: 10, streamedBytes: 10 }
  };
...
test('mismatchDiagnostics enforces allowlisted enum values', () => {
  const item = {
    ...
    _mismatchDiagnostics: { expectedMediaType: 'INJECTED', actualMediaType: 'bad', mimeClass: null, contentTypeToken: 'a'.repeat(100), httpStatus: -1, declaredLength: NaN, streamedBytes: Infinity }
  };
