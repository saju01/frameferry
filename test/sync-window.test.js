...
async function browserFixture(t,status=200,profileDelay=0,mediaLength=jpg.length){
 ...
 const contexts=[];let requests=0,apiRequests=0,preview=0,browserCloses=0,downloads=0;
 ...
 const wrap={newContext:async opts=>{assert.equal(opts.serviceWorkers,'block');const c=await browser.newContext(opts);contexts.push(c);await c.route('**/*',async route=>{
  const u=new URL(route.request().url());if(u.pathname==='/media'){if(route.request().resourceType()!=='fetch'){preview++;return route.abort();}downloads++;return route.fulfill({status:200,contentType:'image/jpeg',headers:{'content-length':String(mediaLength)},body:jpg});}
 ...
 return {chromium:{launch:async()=>wrap,connectOverCDP:async()=>wrap},counts:()=>({requests,apiRequests,preview,browserCloses,downloads}),dnsLookup:async()=>[{address:"93.184.216.34",family:4}],browser};
}
test('full browser discovery -> real downloadOne -> receipts; repeat is cache-only, no history claim',async t=>{
 const root=await tmp(t),fixture=await browserFixture(t);
 ...
 const second=await W.syncWindow({...config,runId:'second',resultFile:path.join(root,'second.json')},fixture);assert.equal(second.status,'COMPLETE');assert.equal(second.totals.reused,1);assert.equal(fixture.counts().downloads,1);assert.equal(fixture.browser.isConnected(),true);
...
test('public provider has no inherited 120/140 quota, and history/denial remain intact',async t=>{
 ...
 assert.throws(()=>b.inspect(403,{},'https://instacognito.com'),/DENIED/);b.close();const c=openBudget(p,'new-policy');c.reserve('discovery');assert.equal(c.data.denial,null);assert.equal(c.data.denial_history[0].status,403);c.close();
});
test('shared admission serializes concurrent starts and denial prevents queued calls',async t=>{
 const p=path.join(await tmp(t),'budget.json'),b=openBudget(p,'pace');let times=[];
 await Promise.all(['discovery','download','discovery'].map((phase,i)=>b.admit(phase).then(()=>times.push(i))));assert.deepEqual(times,[0,1,2]);assert.equal(b.data.min_request_interval_ms,0);
...
test('expired download signal cannot send a queued request',async t=>{
...
test('closing a run rejects queued admissions before ledger/network mutation',async t=>{
...
test('request accounting survives close and reopen without invented pacing',async t=>{
 const p=path.join(await tmp(t),'budget.json'),a=openBudget(p,'run-a');await a.admit('discovery');a.close();
 const b=openBudget(p,'run-b');await b.admit('download');assert.equal(b.data.min_request_interval_ms,0);assert.equal(b.data.recent_request_ms.length,2);assert.equal(b.data.requests,1);b.close();
});
...
test('a queued admission past the job deadline is neither reserved nor forwarded',async t=>{
 ...
 await g.send(c);
 // Advance the wall clock before the queued admission runs: no pacing assumption.
 const queued=g.send(c),now=Date.now();t.mock.method(Date,'now',()=>now+1000);
 await queued;
...
test('a recorded provider denial outranks the job deadline for queued admissions',async t=>{
 ...
 later.reserve('discovery');assert.equal(later.data.denial,null);assert.equal(later.data.denial_history[0].status,403);later.close();
});
...
test('legacy local ceilings become history before a session rollover overwrites them',async t=>{
 ...
 assert.equal(b.data.quota_policy,'public-provider-unpaced-v2');assert.equal(b.data.session_ceiling,99);assert.equal(b.data.hourly_ceiling,null);
 ...
 c.reserve('discovery');assert.equal(c.data.denial,null);assert.equal(c.data.denial_history[0].status,403);c.close();
});
...
test('partial middle handle retains witness gap and later healthy handle downloads',async t=>{
 ...
 assert.equal(fixture.counts().downloads,2);assert.equal(fixture.counts().requests,3);assert.equal(d.totals.downloaded,2);
...
test('byte resource failure stops later handles instead of being isolated',async t=>{
 const root=await tmp(t),fixture=await browserFixture(t,200,0,2048);
...
test('authenticated-view middle hold creates no page or provider/media request and later healthy handle runs',async t=>{
 const root=await tmp(t),fixture=await browserFixture(t),launch=fixture.chromium.launch;let pages=0;
...
// Item 3: refusal precedence at the queued reservation boundary, without pacing.
test('a denial recorded while an admission is queued survives a lapsed deadline',async t=>{
 ...
 // Both stops are already visible when the queue advances; the refusal wins.
 await assert.rejects(queued,e=>e.code==='PROVIDER_DENIED');
 ...
 const later=openBudget(p,'after-denial-race');later.reserve('discovery');assert.equal(later.data.denial_history[0].status,403);later.close();
});
test('a recorded denial publishes BLOCKED from the ledger, not from the escaping error code',async t=>{
 ...
 // An actual unresolved authentication prerequisite, not a generic historical 403.
 await fs.writeFile(ledger,JSON.stringify({version:1,session_id:'earlier',requests:3,blocked:0,session_ceiling:null,by_phase:{discovery:3},recent_request_ms:[Date.now()],denial:{id:'abc123',kind:'DENIED_AUTH',status:401,at:new Date().toISOString(),session_id:'earlier'},prior_sessions:[]}));
...
test('a later lesser stop can never downgrade a recorded provider denial',async t=>{
 ...
 b.close();
 const c=openBudget(p,'precedence');
